...
router.post('/convertUpper', function (req, res) {
...
router.post('/convertLower', function (req, res) {
   //Check if all fields are provided and are valid:
   if (!req.body.json) {
      res.status(400);
      res.json({ message: "Bad Request" });
      ...
      var response = {};
      names.forEach((value) => {
         namesUpper.push(value.toLowerCase());
      });
      address.forEach((value) => {
         addressUpper.push(value.toLowerCase());
      });
      response = {
         names: namesUpper,
         address: addressUpper
